...
class Figure extends Observable implements Runnable{
    ...
    public void run(){
        while(true){
            ...
            if(x == Figures.width - 50)xplus = false;
            if(x == Figures.width - Figures.width)xplus = true;
            if(y == Figures.height - 40)yplus = false;
            if(y == Figures.height - Figures.height + 40)yplus = true;
            if(xplus)x++;else x--;
            if(yplus)y++;else y--;
            setChanged();
            notifyObservers(this);
            try{Thread.sleep(this.speed);}
